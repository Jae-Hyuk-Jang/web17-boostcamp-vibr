...
| [`search.md`](./search.md)       | 검색 UI가 3곳(nav 드로어/글쓰기 모달/플레이리스트 상세 모달)에서 독립 재구현된 실태와, 오버레이 열림 상태가 zustand `useModalStore`/로컬 state로 분산된 원인 |
...
4. 검색 중복을 해소하려면 `search.md`의 제안대로 오버레이 열림 상태 통일 여부부터 먼저 결정한 뒤 UI 계층을 뽑아야 한다 — `docs/refactors/search-widget-duplication/`에서 진행.
