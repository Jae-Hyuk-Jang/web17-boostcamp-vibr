...
원래 결정: 해당 없음 — 3안 모두 자체 구현이며, PRD 목표 인터뷰에서 이미 라이브러리 도입을 불허하기로 확정했다.

### 구현 중 발견한 문제와 결정 변경 (이슈 #104 착수 중)

`Button`을 실제 58개 파일 중 `modals` 배치 1(10개)에 적용해보니, `className` 문자열 단순 이어붙이기(`\`${BASE} ${VARIANT} ${SIZE} ${className}\``)는 "나중에 오는 클래스가 이긴다"를 보장하지 않는다는 것을 컴파일된 CSS로 직접 확인했다. 예: `Button`의 기본 `rounded-md`와 소비처가 넘긴 `className="rounded-full"`을 함께 두면, CSS 파일에서 어느 쪽이 더 뒤에 정의됐는지(= Tailwind 내부 유틸리티 등록 순서, JSX 문자열 순서와 무관)에 따라 승자가 갈린다 — `rounded-md`/`rounded-full`은 기본값이 이기고, `px-6`/`px-8`은 우연히 소비처 값이 이기는 등 **속성마다 결과가 다르다.** Button의 기본값을 실제 코드의 최빈값(`rounded-full` 등)으로 재설계해봐도, 58개 파일이 각자 조금씩 다른 padding/hover를 갖는 이상 구조적으로 안전한 오버라이드 방법이 없다.

**Q. className 오버라이드가 속성별로 이길지 질지 예측 불가능하다는 게 확인됐습니다(구조적 제약). 어떻게 할까요?**

A. `tailwind-merge`만 도입. 이유: 지금 문제를 실제로 해결하는 도구는 `class-variance-authority`(variant 정의를 구조화할 뿐 클래스 충돌은 그대로 남음)가 아니라 `tailwind-merge`(같은 CSS 속성을 겨냥한 클래스가 충돌하면 나중 클래스가 항상 이기도록 보장)다. 가볍고(~3KB gzip) 단일 목적이며 활발히 유지보수된다. `variant`/`size` 객체 매핑 구조는 그대로 유지하고, 클래스 병합 부분만 `twMerge()`로 감싼다.

**PRD 1-4(라이브러리 도입 불허)를 개정한다**: "새 라이브러리 도입을 허용하는가"에 대한 답을 "불허"에서 "`tailwind-merge`(단일 목적 클래스 병합 유틸리티에 한해) 허용"으로 변경한다. 근거: 처음 불허를 결정한 이유("이 저장소는 cn/clsx조차 안 쓰고 Tailwind 클래스를 직접 조합")가 전제로 삼은 "직접 조합이 문제없이 동작한다"는 가정이, 실제 다중 소비처 재사용 컴포넌트(`Button`)에서는 깨진다는 게 구현 중 확인됐다. `ModalPanel`/`ModalCloseButton`처럼 소비처가 1~2개 속성만 얹는 경우엔 충돌 확률이 낮아 문제가 드러나지 않았을 뿐이다.
