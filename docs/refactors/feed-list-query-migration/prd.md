...
- **Fact**: `FeedView`/`ProfileView`/`ProfilePostsFeed` 어디에도 전용 컴포넌트 테스트가 없다(`find . -iname "FeedView.test.*" ...` → 0건). `useUserSearch.test.ts`(2건, #149에서 초기 로드 에러 상태 특성화만 다룸)와 `UserListModal.test.tsx`(2건, #66에서 배경클릭/닫기 버튼만 다루며 `useInfiniteScroll` 자체를 통째로 mock)는 존재하지만, 데이터 로드/페이지네이션/디바운스/팔로우 토글 같은 실제 목록 동작은 검증하지 않는다 — 이슈 1 작성 후 재확인해 정정한 내용이다(초기 draft에서는 5곳 모두 0건이라고 잘못 적었음). `useInfiniteScroll.ts` 자체의 특성화 테스트(#149에서 추가, 5건)는 훅 레벨이라 소비처의 실제 배선(어떤 fetchFn을 넘기는지, `mergeItems`/`resetKey` 조합)까지는 검증하지 않는다.
