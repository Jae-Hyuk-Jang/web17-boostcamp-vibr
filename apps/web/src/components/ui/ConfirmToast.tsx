  ...
  const ConfirmComponent = ({ closeToast }: { closeToast?: () => void }) => (
    <div className="flex flex-col items-center text-center gap-2 p-1 mx-auto">
      ...
      <div className="flex justify-end gap-2 mt-1">
        <button onClick={closeToast} className="px-3 py-1.5 text-xs font-bold text-gray-500 bg-gray-200 rounded hover:bg-gray-200">
          취소
        </button>
        <button
          onClick={() => {
          ...
          className="px-3 py-1.5 text-xs font-bold text-white bg-red-500 rounded hover:bg-red-600"
        >
          확인
        </button>
      </div>
